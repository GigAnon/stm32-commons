...
void handleRxSNIC(uint8_t* buf, int len)
{
    ...
    case SNIC_CONNECTION_RECV_IND: {
        ...
        else if (strncmp((char*)buf+5, "HTTP/1.", 7) == 0)
        { // Receives HTTP response, close socket. If the socket is not closed,
             for (uint32_t i=0; i<sentsize; i++)
                printf("%c", buf[5+i]);
...
int fillNSendHttpMoreReq(int8u seq, int contentLen, char* content, char moreData)
{
    ...
    *((int16u*)&buf[2]) = swap16(len);
    ...
    if ((unsigned)(contentLen+4) > sizeof(buf))
        return -1;
